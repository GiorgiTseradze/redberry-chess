...
const validate = (value, name) => {
  ...
  case "dob":
    if(Number(value.substring(0, 4)) < 2010 && Number(value.substring(0, 4)) > 1900){
      test(name, true);
    } else {
      test(name, false);
    }
  ...
  return (
    <>
      <Routes>
        ...
        <Route path="/info" element={<Info handleChange={handleChange} data={formData}  isFormValid={isFormValid} />} />
        <Route path="/experience" element={<Experience isFormValid={isFormValid} handleChange={handleChange} setFormData={setFormData} validate={validate} />} />
        <Route path="/completion" element={<Completion handleSubmit={handleSubmit} />} />
      </Routes>
